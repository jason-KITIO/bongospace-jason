...
function Ajouter() {
    ...
    const handleSubmit = async (event) => {
        ...
        formData.append('data.attributes.category.data.attributes.Nom', categorie)
        ...
        try {
            const response = await axios.post('http://localhost:1337/api/produits?populate=*', formData, {
                headers: {
                        ...
                        <div className='nom'>
                            ...
                            <div className='deux'>
                                ...
                                <select value={categorie} onChange={(e) => setCategorie(e.target.value)} required>
                                    ...
                                    {categories.map((cat) => (
                                        <option key={cat.id} value={cat.attributes.Nom}>{cat.attributes.Nom}</option>
                                    ))}
                                </select>
                            </div>
                        </div>
