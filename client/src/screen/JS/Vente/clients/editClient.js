...
function EditClient() {
    const { id } = useParams(); // Récupérer l'ID du client à partir des paramètres de l'URL
    ...
    useEffect(() => {
        const fetchClient = async () => {
            try {
                const response = await axios.get(`http://localhost:1337/api/clients/${id}`);
                setClient(response.data.data.attributes);
            } catch (error) {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            await axios.put(`http://localhost:1337/api/clients/${id}`, {
                data: {
                    Nom: client.Nom,
                    Email: client.Email,
            ...
            navigate('/clients'); // Rediriger vers la liste des clients après la mise à jour
        } catch (error) {
    ...
    return (
        <div className="app">
            ...
            <div className='page page-client'>
                <span className='titre-texte'>Modifier le client</span>
                <div className='titre'>
                    <p>Général</p>
                ...
                <div className='block'>
                    <div className='gauche gauche-client'>
                        <div className='nom nom-client'>
                            <div className='un'>
                                <label>Nom</label>
                                <input type="text" id="Nom" name="Nom" value={client.Nom} onChange={handleInputChange} />
                            </div>
                            <div className='un'>
                                <label>Email</label>
                            ...
                            <div className='un'>
                                ...
                                <input type="text" id="Telephone" name="Telephone" value={client.Telephone} onChange={handleInputChange} />
                            </div>
                        </div>
                        <div className='nom nom-client'>
                            <div className='un'>
                                <label>Ville</label>
                                <input type="text" id="Ville" name="Ville" value={client.Ville} onChange={handleInputChange} />
                            </div>
                            <div className='un'>
                                <label>Quartier</label>
                                <input type="text" id="Quartier" name="Quartier" value={client.Quartier} onChange={handleInputChange} />
                            </div>
                        </div>
                    </div>
